...
export function AvatarBadge({
  ...
  const sizeClass = size === 'sm' ? 'h-9 w-9 text-[10px]' : 'h-12 w-12 text-xs'

  return (
    <span
      className={`inline-flex ${sizeClass} items-center justify-center rounded-2xl ring-2 ${avatar.bg} ${avatar.ring} ${avatar.text} font-display font-bold uppercase tracking-[0.16em] shadow-[inset_0_2px_0_rgba(255,255,255,0.7),0_6px_14px_rgba(15,23,42,0.16)]`}
      title={avatar.label}
  ...
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className} aria-hidden="true">
      <path
        ...
        strokeWidth="1.4"
      />
      <path d="M5.5 20h13" className="stroke-amber-800" strokeWidth="1.6" strokeLinecap="round" />
    </svg>
  )
}
  ...
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className} aria-hidden="true">
      <path
        ...
        className="fill-sky-300 stroke-sky-700"
        strokeWidth="1.4"
      />
      <path d="M8.5 8h7M12 3v15" className="stroke-white/70" strokeWidth="1.4" />
    </svg>
  )
}
...
export function HourglassIcon({ className = 'h-5 w-5' }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className} aria-hidden="true">
      <path d="M8 4h8M8 20h8" className="stroke-emerald-900" strokeWidth="1.6" strokeLinecap="round" />
      <path
